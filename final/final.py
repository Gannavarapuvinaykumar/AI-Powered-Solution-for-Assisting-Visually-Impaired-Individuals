...
# Loading API key
with open(r"C:\Users\ganna\PycharmProjects\JarvisAI\AI Code Reviewer\keys.txt") as f:
    api_key = f.read()

genai.configure(api_key=api_key)

# Streamlit App Configuration
st.set_page_config(page_title="AI-Powered Visual Support", layout="centered", page_icon="🌟")
...
# Identify and Highlight Objects in Image
...
# Streamlit interface
st.title("Live Video Assistance for the Visually Impaired")
if st.button("Begin Video Recording"):
    process_video()

# Sidebar with Buttons
st.sidebar.title("Actions to Explore")
text_button = st.sidebar.button("Describe Image 📜")
scene_button = st.sidebar.button("Extract and Read Text 🖋️")
detect_button = st.sidebar.button("Identify and Highlight Objects 🚦")
assist_button = st.sidebar.button("Provide Personalized Guidance")
audio_button = st.sidebar.button("Pause Audio Playback 🔇")

# Center Layout for Upload Section
st.title("Vision Empowerment Tool for the Visually Impaired")
...
for uploaded_image in uploaded_images:
    ...
    if assist_button:
        with st.spinner("Providing assistance..."):
            ...
            image_bytes = convert_image_to_bytes(uploaded_image)
            assistance_response = genai.GenerativeModel("gemini-1.5-pro").generate_content(
                [assistance_prompt, image_bytes[0]]).text
